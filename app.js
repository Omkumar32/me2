...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function loadConfigSocials() {
    try {
      ...
      // Update Bio Profile Card details dynamically
      if (config.profile) {
      ...
      if (featuredRow && projects.length > 0) {
        featuredRow.innerHTML = projects.map(p => {
          ...
          if (titleLower.includes("cricket")) {
            ...
            iconName = "file-text";
            iconClass = "pdf";
          } else if (titleLower.includes("medication") || titleLower.includes("tracker") || titleLower.includes("health")) {
            iconName = "pill";
            iconClass = "med";
          }

          const tagsHtml = (p.tech || []).slice(0, 2).map(tag => `<span>${tag}</span>`).join('');
      ...
      if (projectsGrid && projects.length > 0) {
        projectsGrid.innerHTML = projects.map((p, idx) => {
          ...
          const techHtml = (p.tech || []).map(tag => `<span>${tag}</span>`).join('');
      ...
      // Re-initialize 3D card tilt & mouse glow border on new elements
      ...
      // Bind interactive cursor follow glow on new elements
      const newInteractives = document.querySelectorAll(".mini-project-card, .project-card, .btn");
      ...
      newInteractives.forEach(el => {
        el.addEventListener("mouseenter", () => {
          if (cursorGlowEl) {
            ...
            cursorGlowEl.style.background = "radial-gradient(circle, rgba(200, 255, 61, 0.08) 0%, rgba(45, 27, 105, 0.05) 50%, rgba(0,0,0,0) 70%)";
          }
        });
        el.addEventListener("mouseleave", () => {
          if (cursorGlowEl) {
            ...
            cursorGlowEl.style.background = "radial-gradient(circle, rgba(200, 255, 61, 0.05) 0%, rgba(45, 27, 105, 0.03) 50%, rgba(0,0,0,0) 70%)";
          }
        });
      });
    ...
    contactForm.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        if (res.ok && data && data.success) {
          ...
          const errMsg = data ? (data.error || data.details) : `Server error (Status ${res.status})`;
  ...
  function handleRouting() {
    ...
    let sectionId = "home"; // default fallback
    ...
    // Map "/github" to "github-stats" section ID
    if (sectionId === "github") {
  ...
  document.addEventListener("click", e => {
    ...
    if (link && link.href) {
      ...
      if (url.origin === window.location.origin) {
        ...
        // Don't intercept admin, api or static files
        if (path === "/admin" || path.startsWith("/api") || path.endsWith(".pdf") || path.startsWith("/uploads")) {
          return;
        }

        // List of section paths
        const sectionsList = ["/about", "/skills", "/projects", "/experience", "/github", "/contact", "/home", "/github-stats"];
  ...
  // Run on initial page load (with a tiny delay to ensure GSAP/rendering is complete)
  setTimeout(handleRouting, 600);

  /* ==========================================================================
     FLOATING TECH STACK INTERACTION SYSTEM
     ========================================================================== */
  ...
  if (floatContainer) {
    ...
    // Initialize bubble elements and state
    techBubblesData.forEach((data, index) => {
      ...
      // Random position setup inside container boundaries
    ...
    // Setup drag and drop events for bubbles
    bubbles.forEach(b => {
      ...
      const endDrag = () => {
        if (!b.isDragging) return;
        b.isDragging = false;
        // Toss momentum with fallback limit
    ...
    function updatePhysics() {
      ...
      bubbles.forEach(b => {
        if (b.isDragging) {
          // Render position during drag
          b.el.style.transform = `translate(${b.x - b.radius}px, ${b.y - b.radius}px)`;
          return;
        }

        // Apply constant organic slow motion (damping deceleration or minimal velocity force)
        b.vx *= 0.98;
        b.vy *= 0.98;

        // Keep minimum drifting speed
        ...
        // Dynamic boundaries & physics update
        b.x += b.vx;
        b.y += b.vy;
